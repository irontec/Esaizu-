...
class upload {
    ...
    function upload($file, $lang = 'en_GB') {
        ...
        if ($this->uploaded) {
            ...
            if ( false !== ($pos = strpos($this->file_src_mime, ";")) ) {

                $this->file_src_mime = substr($this->file_src_mime, 0, $pos);
            }
            
            // determine whether the file is an image
            echo $this->file_src_mime."#<br/>";
            echo $this->file_src_mime."#<br/>";
            print_r($this->image_supported);

            
            
            
            
            if ($this->file_src_mime && is_string($this->file_src_mime)
            && !empty($this->file_src_mime)
            && array_key_exists($this->file_src_mime, $this->image_supported)) {
